...
func HandlerAddUser(w http.ResponseWriter, r *http.Request, useCase usecase.UseCaseStorage, conf *config.Config) {
	...
	if err := dec.Decode(&req); err != nil {
		logger.SugaredLogger().Errorw("Ошибка декодирования тела запроса", "error", err)
...
// @Failure 400 {string} string "Пустой ID"
// @Failure 422 {string} string "Ошибка конвертирования"
// @Failure 500 {string} string "Ошибка сервера"
// @Router /delUser/{userID} [delete]
func handlerDelete(w http.ResponseWriter, r *http.Request, useCase usecase.UseCaseStorage) {
	userIDstr := chi.URLParam(r, "userID")
	if userIDstr == "" {
		logger.SugaredLogger().Error("Не указан ID пользователя")
		w.WriteHeader(http.StatusBadRequest)
		return
	}
...
// @Failure 422 {string} string "Ошибка валидации серии или номера паспорта"
// @Failure 500 {string} string "Ошибка сервера"
...
// @Failure 400 {string} string "Не указан ID пользователя или ID не является числом"
// @Failure 422 {string} string "Ошибка конвертирования ID"
// @Failure 500 {string} string "Ошибка сервера"
...
// @Failure 400 {string} string "Не указан ID пользователя или неверный формат параметров периода времени"
// @Failure 500 {string} string "Ошибка сервера"
